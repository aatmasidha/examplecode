...
public class RateFetchCSVTest {
	...
	private static final Object[] FILE_HEADER = { "hotelname", "hotelcode",
			"otacode", "dtcollected", "ratedate", "los", "checkindate",
			"checkoutdate", "netrate", "onsiterate", "roomtype",
			"ispromotional", "ratetype", "discount", "closed", "currency" };
	...
	public static void main(String[] args) {
		...
		try {
			restFetchTest.fetchAllRates(90, context);
	...
	public void fetchAllRates(int reqNumDays, ApplicationContext context)
		...
		try {
			...
			for (CSVRecord hotelRecord : hotelRecords) {
				...
				row++;
				Date date = checkinDateFormat.parse("2016-12-17");
				String checkInDateString = checkinDateFormat.format(date);
				// String checkInDateString = checkinDateFormat.format(new
				// Date());
				...
				RateShoppingPropertyDetailsDTO rateShoppingPropertyDetailsDTO = rateShoppingPropertyDetailsMappingService
						.findRateShoppingDetailsForPropertyByPropertyCode(
								rateShoppingPropertyUID, "2");
				if (rateShoppingPropertyDetailsDTO == null) {
					logger.error("No Rate Shopping data defined for property: "
							+ hotelRecord.get(0) + "With PropertyCode:"
							+ hotelRecord.get(1));
					throw new RateShoppingDataException(
							"No Rate Shopping data defined for property: "
									+ hotelRecord.get(0) + "With PropertyCode:"
									+ hotelRecord.get(1));
				}
				...
				for (int numDays = 0; numDays < reqNumDays; numDays++) {
					...
					hotelRequestDTO.setCurrency("GBP");
					...
					HotelRateThreadCSV hoteRateThread = new HotelRateThreadCSV(
							hotelRequestDTO, hotelRecord.get(0), processDate,
							context);
				...
				String rateShoppingCsvFiles = ConnectionProperties
						.getProperties().getProperty("rateshoppingbasefolder")
						+ File.separator
						+ rateShoppingPropertyUID
						+ File.separator + processDate;
				...
				String filePath = basePath + File.separator
						+ "ratesummaryreport" + File.separator + processDate
						+ File.separator;
				String pathToSummaryFiles = filePath + "Summary_"
						+ hotelRecord.get(0) + "_" + rateShoppingPropertyUID
						+ "_" + processDate + ".csv";
